...
- **The tiers do not overlap.** Adding a keyword band-wide supersedes it: any personal copy is removed, because the band keyword already posts the entry and the personal duplicate would only add a redundant ping. The reverse is allowed on purpose, so a member can add a band keyword to their own list precisely to be @mentioned for it. Running `/warband interests add` on a keyword that is already shared still sweeps up personal copies, so it repairs an existing overlap.
